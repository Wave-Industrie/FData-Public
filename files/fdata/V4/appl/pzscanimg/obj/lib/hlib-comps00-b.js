...
//-----
 this.newTable2 = function(d0,appl_defines,global_defines,lib,ohneScroll)
  {
  return(new this.comps.Table2(d0,appl_defines,global_defines,lib,ohneScroll));
  };
...
//-----
 this.newEventPopup = function(evnt,origin,evntfct)
  {
  var o = this.getEventXYonBody(evnt);
//log("x="+o.X+" y="+o.Y);
  return(new HlibComps00A.Popup(o.X,o.Y,origin,evntfct));
  };
*/
//-----
...
//-----
 this.registerResizer = function(c,fct)
  {
  c["do-resize"] = fct;
  this.RESIZECOMPS.push(c);
  return(this);
  }
...
//-----
 this.resizeElements = function()
  {
  for (var i = 0; i < this.RESIZECOMPS.length; i++) this.RESIZECOMPS[i]["do-resize"]();
  return(this);
  }
...
*/
  ...
  if (e0.className.indexOf("jt3") >= 0)
   {
   if (e0.className.indexOf("noscroll") >= 0)
    this.newTable2(e0,defs.ID[e0.id],defs,this,true);
   else
    this.newTable2(e0,defs.ID[e0.id],defs,this,false);
   return;
   }
  ...
  if (e0.className.indexOf("e22ng") >= 0)
   {
   new HlibComps00A.E22ng(e0,this);
   return;
   }

  if (e0.className.indexOf("buti") >= 0)
   {
   if (defs.ID[e0.id] && defs.ID[e0.id].POPUP)
    {
    e0.onclick = function(evnt)
     {
     var pp = this.onclick.DEFS.POPUP;
     if (!pp) return;
     var evntfct = pp[0]();
     if (!evntfct) return;
     var p = this.lib.newEventPopup(evnt,null,evntfct);
     for (var i = 1; i < pp.length-1; i+=2) p.neu(pp[i],pp[i+1]);
     p.show();
     }
    e0.onclick.DEFS = defs.ID[e0.id];
    e0.lib = this;
    }
   else
    {
  ...
  if (e0.className.indexOf("dblk") >= 0)
   {
   e0.TIMEOUT_ID = null;

   this.setStyle(e0,"position","absolute","overflow","hidden","top","0px","left","0px","width","100%","height","100%","zIndex","1","cursor","");
   var w = document.createElement("DIV");
   this.setStyle(w,"position","absolute","overflow","hidden","bottom","0px","left","0%","width","10%","height","5px","backgroundColor","cyan"); //"#3a6ea5");
   e0.appendChild(w);

   e0.showWaiting = function(b)
    {
    if (b)
     {
     if (this.TIMEOUT_ID != null) return; //(alert("schon running"));
     this.style.zIndex = 98;
     this.style.cursor = "wait";
     this.TIMEOUT_ID = setTimeout("document.getElementById('"+this.id+"').timerReceived()",25);
     }
    else
     {
     this.style.zIndex = 0;
     this.style.cursor = "";
     if (this.TIMEOUT_ID == null) return; //(alert("nicht running"));
     clearTimeout(this.TIMEOUT_ID);
     this.TIMEOUT_ID = null;
     }
    };

   e0.timerReceived = function()
    {
    this.TIMEOUT_ID = null;
    if (this.style.zIndex != 98) return;
    var w = this.firstChild;
    var x = w.style.left.replace("%","");
    x = 1*x+1;
    if (x >= 90) x = 0;
    w.style.left = ""+x+"%";
  ...
  if (e0.className.indexOf("laycenter") >= 0) this.registerLayouter(e0,this.doCenter);
...
// =======================================================================
_comps_.Table2 = function(p0,appl_defines,all_defines,lib,ohneScroll)
 {
 HlibStdlib.call(this);

 this.DEFS = appl_defines;
...
//------------------------------------------------------------------------
 this.onRowClicked = function(evnt,r0)
  {
  if (this.L) this.L(evnt,r0);
  };

 this.newEventPopup = HlibComps00A.prototype.newEventPopup;

//------------------------------------------------------------------------
 this.showPopup = function(evnt,r0)
  {
  ...
  var evntfct = pp[0]();
  if (!evntfct) return;
  var p = this.newEventPopup(evnt,null,evntfct);
  for (var i = 1; i < pp.length-1; i+=2) p.neu(pp[i],pp[i+1]);
  p.show();
  };

//------------------------------------------------------------------------
 this["set-popup-menu"] = function(e)
  {
  var nl = e.childNodes;
  var pp = this.DEFS.POPUP;
...
// this.setStyle(d0,"position","absolute","height",""+d0h+"px","fontSize",""+fh+"px","fontFamily","arial","fontWeight","bold").setStyle(d1,"position","absolute","overflowY","scroll").setStyle(jt,"width","100%","fontSize",""+fh+"px","fontFamily","arial","fontWeight","normal","tableLayout","fixed");
 this.setStyle(d0,"position","absolute","height",""+d0h+"px","fontSize",""+fh+"px","fontFamily","arial","fontWeight","bold");
 if (ohneScroll == true)
  this.setStyle(d1,"position","absolute");
 else
  this.setStyle(d1,"position","absolute","overflowY","scroll");
 this.setStyle(jt,"width","100%","fontSize",""+fh+"px","fontFamily","arial","fontWeight","normal","tableLayout","fixed");
 d1.appendChild(jt);
 ...
 if (!appl_defines.NOCLICK)
  ...
  b.onclick = function(evnt)
   ...
   if (!evnt.ctrlKey)
    {
    p0.onRowClicked(evnt,src);
    p0.showPopup(evnt,src);
    }
   };
  }
...
// var gn = APPL_DEFINES.ID[p0.id].ALIGN;
 ...
 for (var i = 0; i < tt.length; i++)
  ...
  if (ttid && (ttid[i] != null))
   ...
   if (all_defines.buttonOnclick) d.onclick = all_defines.buttonOnclick;
...
//  this.colorRows();
 ...
 lib.registerResizer(this,this.doLayout);
 };
...
// --------------------------------------------------------------
 this["clear-table"] = function(e0)
  {
  this.LAST_scrollTop = this.P0.scrollTop;
...
// --------------------------------------------------------------
 this["do-layout"] = function(e0)
  {
  for (var i = 0; i < this.ELES.length; i++) this.addElement(this.ELES[i]);
  this.ELES = new Array();
  ...
  var st = e0.getAttribute("scroll-top");
  if (st == null)
   p0.scrollTop = this.LAST_scrollTop;
  else
   p0.scrollTop = 1*st;
  };
...
//------------------------------------------------------------------------
 this.showPopup = function(evnt,r0) // to-do: in attr von r0 punkte definieren
  {
  var pp = this.DEFS.POPUP;
  if (!pp) return;
  var evntfct = pp[0]();
...
// --------------------------------------------------------------
 this.onclick = function(evnt)
  ...
  if (src.DATA_ELEMENT.getAttribute("typ") == "title") return;

  this.setSelected(src,evnt.ctrlKey);
  if (!evnt.ctrlKey)
   {
   this.onRowClicked(evnt,src);
   this.showPopup(evnt,src);
   }
  };

//------------------------------------------------------------------------
 this.setSelected = function(r0,isClear)
  {
  if ((r0 == null) || (r0 != this.SELECTED_TR) || (isClear && (r0 == this.SELECTED_TR)))
   {
   if (this.SELECTED_TR)
    {
    this.SELECTED_TR.style.backgroundColor = "white";
    this.SELECTED_TR = null;
    }
   }
  if (isClear || (r0 == null)) return;
  this.SELECTED_TR = r0;
  r0.style.backgroundColor = "cyan";
  };
 ...
 p0["do-layout"] = function(e,cmd)
  {
  var p = this.HLIB_DEFINES.JSELE;
  p["do-layout"](e,cmd);
  };
 p0.onclick = function(evnt)
  {
  this.HLIB_DEFINES.JSELE.onclick(evnt);
  };
...
// =========================================================================
HlibComps00A.E22ng = function(e22,lib)
 {
 HlibStdlib.call(this);
 ...
 var f = function(e0)
  {
  this.JSELE.setText(this,e0.getAttribute("text"));
  this.JSELE.lay();
  };

 this.removeTextChilds(e22);
 var nl = e22.childNodes;
 e22.JSELE = this;
 for (var i = 0; i < nl.length; i++)
  {
  var e = nl[i];
...
/*
 e22["do-xml-command"] = function(e0)
  {
  this.JSELE["do-xml-command"](e0);
  };

 this["do-xml-command"] = function(e0)
  {
  var cmd = e0.nodeName;
log("do-xml-command.cmd="+cmd);
  };
*/

 this.lay = function()
  {
  var w0 = this.E22.clientWidth;
  var h0 = this.E22.clientHeight;
  ...
  if (e)
   ...
   if (wl) wl += 10;
   e.style.left = "0px";
   e.style.top = ""+Math.ceil((h0-e.clientHeight)/2)+"px";
   }
  var e = this.RET;
  if (e)
   {
 ...
 lib.registerLayouter(e22,f);
 };


//-----
HlibComps00A.prototype.newEventPopup = function(evnt,origin,evntfct)
 {
 var o = this.getEventXYonBody(evnt);
 if (!origin)
  {
  var bdy = this.getBody();
  origin = (o.Y > bdy.clientHeight/2) ? "u" : "o";
  origin += (o.X > bdy.clientWidth/2) ? "r" : "l";
  }
 return(new HlibComps00A.Popup(o.X,o.Y,origin,evntfct));
 };
